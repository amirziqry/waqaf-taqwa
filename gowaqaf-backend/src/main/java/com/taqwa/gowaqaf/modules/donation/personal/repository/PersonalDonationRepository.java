...
public interface PersonalDonationRepository extends JpaRepository<PersonalDonation, UUID> {
	...
	Optional<PersonalDonation> findByPersonal_IdAndTransaction_BillingCode(UUID personalId, String billingCode);
	...
	@Query("""
			SELECT new com.taqwa.gowaqaf.modules.donation.personal.dto.PersonalCollectionSum(
			    COALESCE(SUM(CASE WHEN d.transaction.donationType = 'DIRECT' THEN d.transaction.amount ELSE 0 END), 0),
			    COALESCE(SUM(CASE WHEN d.transaction.donationType = 'RECURRING' THEN d.transaction.amount ELSE 0 END), 0),
			    COALESCE(SUM(CASE WHEN d.transaction.donationType = 'PROJECT' THEN d.transaction.amount ELSE 0 END), 0)
			)
			FROM PersonalDonation d
			WHERE d.transaction.status = 'PAID'
			  AND (:startDate IS NULL OR d.transaction.paidAt >= :startDate)
	...
	@Query("""
			...
			FROM (
			    SELECT pd.transaction.amount AS amount
