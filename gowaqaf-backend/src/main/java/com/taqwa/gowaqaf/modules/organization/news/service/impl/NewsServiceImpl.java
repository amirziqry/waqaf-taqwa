...
import com.taqwa.gowaqaf.modules.organization.news.component.image.entity.NewsImage;
...
import com.taqwa.gowaqaf.modules.organization.news.mapper.NewsMapper;
...
public class NewsServiceImpl implements NewsService {
	...
	public NewsUploadResponse createNews(NewsUploadRequest dto) {
		...
		if (dto.getCategory() != null) {
			NewsCategory category = newsCategoryRepository.findById(dto.getCategory().getId()).get();
			news.setCategory(category);
		}
		
		if (dto.getTags() != null) {
			Set<Long> tagIds = dto.getTags().stream().map(tag -> tag.getId()).collect(Collectors.toSet());
			Set<NewsTag> tags = new HashSet<>(newsTagRepository.findAllById(tagIds));
			news.setTags(tags);
		}
