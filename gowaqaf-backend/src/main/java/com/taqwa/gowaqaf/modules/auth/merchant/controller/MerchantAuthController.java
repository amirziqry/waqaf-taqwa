package com.taqwa.gowaqaf.modules.auth.merchant.controller;
...
import com.taqwa.gowaqaf.modules.auth.merchant.dto.MerchantLoginResponse;
...
@RestController
@RequestMapping(ApiBasePath.MERCHANT + "/auth")
...
public class MerchantAuthController {
	...
	@PostMapping("/login")
	public ResponseEntity<MerchantLoginResponse> login(@RequestBody MerchantLoginCredentials request,
			HttpServletResponse response) {
		MerchantAuthDetails auth = authService.login(request);

		ResponseCookie cookie = authCookieManager.createAccessTokenCookie(auth.getToken(), 60 * 15);

		response.addHeader(HttpHeaders.SET_COOKIE, cookie.toString());

		MerchantLoginResponse dto = new MerchantLoginResponse(auth.getUsername(), auth.getEmail());
