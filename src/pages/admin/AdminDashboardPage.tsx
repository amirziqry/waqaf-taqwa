...
const DEFAULT_RAKAN_QR: RakanQrApp[] = [
  {
    agentCode: 'DUTA-4402',
    username: 'ustaz_azhar',
    fullName: 'Ustaz Azhar Ghazali',
    email: 'azhar.ghazali@gmail.com',
    phone: '012-9981122',
    placementLocation: 'Surau Al-Hidayah Mukim 4',
    placementType: 'Surau / Masjid Komuniti',
    shippingAddress: 'Lot 14, Kampung Melayu, 63000 Cyberjaya, Selangor',
    status: 'PENDING',
...
export const AdminDashboardPage: React.FC = () => {
  ...
  const handleApproveDuta = (agentCode: string) => {
    ...
    const targetItem = updated.find((i) => i.agentCode === agentCode);
    if (targetItem?.username) {
      localStorage.setItem(`wt_rakan_qr_app_${targetItem.username}`, JSON.stringify(targetItem));
    }

    const currentActiveApp = localStorage.getItem('wt_rakan_qr_application');
    if (currentActiveApp) {
      try {
        const parsed = JSON.parse(currentActiveApp);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      {/* PURE ADMIN TOPBAR - COMPLETELY SEPARATE FROM USER TABS */}
      <header className="bg-[#0F2028] text-white sticky top-0 z-40 px-4 md:px-8 py-4 shadow-sm">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="font-black text-lg tracking-tight text-white">Waqaf Taqwa</span>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              type="button"
              onClick={() => navigate('/')}
              className="flex items-center gap-1.5 px-3.5 py-2 bg-white/10 hover:bg-white/15 text-slate-200 rounded-xl text-xs font-bold transition"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              <span>Ke Laman Pewakaf</span>
            </button>

            <button
              type="button"
              onClick={handleAdminLogout}
              className="flex items-center gap-1.5 px-3.5 py-2 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 rounded-xl text-xs font-bold transition"
            >
      ...
      <main className="max-w-6xl mx-auto space-y-6 p-4 md:p-6 pb-20">
        {/* Banner Action Bar */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Kawalan kelulusan kempen waqaf, pendaftaran rakan niaga, kit standee, dan kakitangan sistem
            </p>
          </div>
        ...
        {/* TAB 3: Permohonan Ahli Duta */}
        {activeTab === 'rakanqr' && (
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden">
            <div className="p-5 border-b border-slate-100">
              <h3 className="font-extrabold text-[#0F2028] text-base">Permohonan Ahli Duta & Standee QR</h3>
              <p className="text-xs text-slate-400">Klik pada baris pemohon untuk melihat maklumat terperinci dan alamat penghantaran</p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/75 border-b border-slate-100 text-[11px] font-extrabold text-slate-500 uppercase tracking-wider">
                    <th className="py-3.5 px-5">Nama Pemohon</th>
                    <th className="py-3.5 px-4">E-mel</th>
                ...
                <tbody className="divide-y divide-slate-100 text-xs font-semibold">
                  {rakanQrApps.map((item) => {
                    ...
                    const isExpanded = !!expandedRows[item.agentCode];

                    return (
                      <React.Fragment key={item.agentCode}>
                        <tr 
                          onClick={() => toggleExpandRow(item.agentCode)}
                          className="hover:bg-slate-50/75 transition cursor-pointer"
                        >
                          <td className="py-4 px-5">
                            <p className="font-extrabold text-slate-800">{item.fullName}</p>
                            <span className="text-[10px] text-slate-400 font-mono">@{item.username || 'user'}</span>
                          </td>
                          <td className="py-4 px-4 text-slate-600">
                            {item.email || `${item.username || 'pemohon'}@taqwa.com`}
                          </td>
                          <td className="py-4 px-4 text-slate-700 font-mono">
                            {item.phone || item.phoneNumber || 'Tiada No. Telefon'}
                          </td>
                          <td className="py-4 px-4">
                            <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase ${
                          ...
                          <tr className="bg-slate-50/60 border-y border-slate-100">
                            <td colSpan={6} className="px-6 py-4">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                                <div className="space-y-1.5 bg-white p-3.5 rounded-2xl border border-slate-200/70">
                                  <div className="flex items-center gap-1.5 text-slate-400 text-[10px] uppercase font-bold">
                                    <Tag className="w-3 h-3" /> Kod Rujukan Duta
                                  </div>
                                  <p className="font-mono font-black text-amber-900 text-sm">{item.agentCode}</p>
                                  <span className="text-[10px] text-slate-400">Taraf: Ahli Duta Taqwa</span>
                                </div>

                                <div className="space-y-1.5 bg-white p-3.5 rounded-2xl border border-slate-200/70">
                                  <div className="flex items-center gap-1.5 text-slate-400 text-[10px] uppercase font-bold">
                                    <MapPin className="w-3 h-3" /> Lokasi Penempatan Standee
                                  </div>
                                  <p className="font-bold text-slate-800">
                                    {item.placementLocation || item.locationName || 'Lokasi Premis Tidak Dinyatakan'}
                                  </p>
                                  <span className="text-[10px] text-slate-500">
                                    Jenis: {item.placementType || 'Premis / Komuniti'}
                                  </span>
                                </div>

                                <div className="space-y-1.5 bg-white p-3.5 rounded-2xl border border-slate-200/70">
                                  <div className="flex items-center gap-1.5 text-slate-400 text-[10px] uppercase font-bold">
                                    <Calendar className="w-3 h-3" /> Alamat Pengeposan Kit Fizikal
                                  </div>
                                  <p className="text-slate-700 font-normal leading-relaxed text-[11px]">
                                    {item.shippingAddress || item.deliveryAddress || 'Tiada alamat pos dinyatakan'}
                                  </p>
                                </div>
                              </div>
                            </td>
