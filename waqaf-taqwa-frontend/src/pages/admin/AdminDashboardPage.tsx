...
export const AdminDashboardPage: React.FC = () => {
  const [isAdminAuth, setIsAdminAuth] = useState(() => {
    return localStorage.getItem("wt_user_role") === "admin";
  });

  const [activeTab, setActiveTab] = useState<
    "campaigns" | "tijarah" | "rakanqr" | "admins"
  >("campaigns");
  ...
  const [rakanQrApps, setRakanQrApps] = useState<any[]>([]);
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const response = await api.get("/organization/project/category");
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const projects = await getAllProjects(0, 100);

        setCampaigns(projects.projectList);
      } catch (error) {
  ...
  // Get all rakanqrs
  useEffect(() => {
    const loadRakanQrs = async () => {
      try {
        const rakanQrs = await getAllRakanQrs(0, 100, undefined, "DUTA"); // filter size and status.
        setRakanQrApps(rakanQrs.rakanQrList);
      } catch (error) {
        console.error("Failed to load Rakan QRs:", error);
      }
    };
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const collectionData = await getDonationCollections();
    ...
    const storedTijarah: TijarahApp[] = JSON.parse(
      localStorage.getItem("wt_admin_vendors") || "[]",
    );
    setTijarahApps(
      storedTijarah.length > 0 ? storedTijarah : DEFAULT_TIJARAH_APPS,
    );

    // const storedRakanApps: RakanQrApp[] = JSON.parse(
    ...
    const storedAdmins = JSON.parse(
      localStorage.getItem("wt_admin_users") || "[]",
    );
  ...
  const handleCreateCampaign = (e: React.FormEvent) => {
    ...
    const primaryImage =
      uploadedImages.length > 0 ? uploadedImages[0] : defaultCover;
    ...
    const created: Campaign = {
      ...
      description:
        ...
        "Pembangunan kemudahan dan kebajikan ummah berterusan.",
      // image: primaryImage,
      image: undefined,
  ...
  const handleDeleteCampaign = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm("Adakah anda pasti mahu memadamkan kempen ini?"))
      return;
  ...
  const handleUpdateTijarahStatus = (
    id: string,
    newStatus: "APPROVED" | "REJECTED",
  ) => {
    const updated = tijarahApps.map((item) =>
  ...
  //   0,
  // );
  const totalFunds = Object.values(collections).reduce(
    (total: number, value: any) => total + Number(value || 0),
    0,
  );
  const pendingTijarahCount = tijarahApps.filter(
    (a) => a.status === "PENDING",
  ).length;
  const pendingRakanQrCount = rakanQrApps.filter(
    (a) => a.status === "PENDING" || !a.status,
  ).length;
  const pendingAdminCount = adminUsers.filter(
    (a) => a.status === "PENDING",
  ).length;
  ...
  const handleCreateCategory = async (
    ...
    try {
      const response = await api.post("/organization/project/category", {
        name,
      });

      // Add newly-created category to the dropdown
  ...
  const createProjectHandler = async (e: React.FormEvent) => {
    ...
    const projectUploadRequest = {
      name: newCampaign.title,
      slugUrl: newCampaign.title.trim().toLowerCase().replace(/\s+/g, "-"),
      category: {
        id: newCampaign.category,
        name: null,
      },
      targetAmount: Number(newCampaign.targetAmount),
      location: newCampaign.location,
      tags: [],
      summary: newCampaign.description,
      contentHtml: "<p>Place content here.</p>",
      status: "PUBLISHED",
      imageUploadRequests: uploadedImages.map((file) => ({
        filename: file.name,
    ...
    try {
      const response = await createProject(projectUploadRequest);

      // TODO: Use returned response to upload project images and update back to backend.
      console.log("Project created:", response);

      // 2. Upload images to object storage
      if (response.imageUploadList.length > 0) {
        await uploadProjectImages(response.imageUploadList, uploadedImages);

        // 3. Save image keys to project
        await updateProjectImageKeys(
          response.projectId,
          response.imageUploadList.map((upload: any) => upload.imageKey),
        );
      }
  ...
  const uploadProjectImages = async (
    imageUploadList: { uploadUrl: string; imageKey: string }[],
    files: File[],
  ) => {
    await Promise.all(
      imageUploadList.map(async (upload) => {
        // Get filename from imageKey
        const filename = upload.imageKey.split("/").pop() ?? "";

        // Remove the unique ID prefix from the filename.
        // Adjust this regex to match your backend's actual key format.
        const originalFilename = filename.replace(/^[a-zA-Z0-9]{8}-/, "");
        const file = files.find((file) => file.name === originalFilename);

        if (!file) {
          throw new Error(
            `Could not find local file for image key: ${upload.imageKey}`,
          );
        }
  ...
  // ADDED
  const approveRakanQrAppHandler = async (id: string, status: string) => {
    try {
      await updateRakanQrStatus(id, status);

      // Refresh the list from backend
      const rakanQrs = await getAllRakanQrs(0, 100, undefined, "DUTA");
      setRakanQrApps(rakanQrs.rakanQrList);
    } catch (error) {
      console.error("Failed to update Rakan QR status:", error);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Kawalan kelulusan kempen waqaf, pendaftaran rakan niaga SSM, kit
            fizikal, dan kakitangan sistem
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xs">
          ...
          <p className="text-xl font-black text-[#0F2028] mt-2">
            RM{" "}
            {totalFunds.toLocaleString("ms-MY", { minimumFractionDigits: 2 })}
          </p>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xs">
          ...
          <p className="text-xl font-black text-blue-600 mt-2">
            {pendingRakanQrCount} Menunggu
          </p>
          <span className="text-[10px] font-bold text-slate-400 mt-1 block">
            Kelulusan Duta & Elaun
          </span>
        </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xs">
          ...
          <p className="text-xl font-black text-purple-700 mt-2">
            {pendingAdminCount} Menunggu
          </p>
          <span className="text-[10px] font-bold text-slate-400 mt-1 block">
            Kebenaran Staf Baharu
          </span>
        </div>
      </div>
      ...
      {activeTab === "campaigns" && (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="font-extrabold text-[#0F2028] text-base">
                Senarai Kempen & Projek Aktif
              </h3>
              <p className="text-xs text-slate-400">
                Pengurusan perolehan infaq dan status paparan awam
              </p>
            </div>
          </div>
                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            type="button"
                            onClick={(e) =>
                              handleDeleteCampaign(String(camp.id), e)
                            }
      ...
      {activeTab === "tijarah" && (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="font-extrabold text-[#0F2028] text-base">
              Permohonan Rakan Tijarah (Peniaga SSM)
            </h3>
            <p className="text-xs text-slate-400">
              Pengesahan premis perniagaan untuk pengaktifan modul infaq kaunter
            </p>
          </div>
                ...
                {tijarahApps.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-4 px-4 max-w-xs truncate">
                      <span className="px-2 py-0.5 bg-slate-100 rounded text-[10px] block w-fit mb-1">
                        {item.category}
                      </span>
                      ...
                      {item.status === "PENDING" ? (
                        <div className="flex items-center justify-end gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              handleUpdateTijarahStatus(item.id, "APPROVED")
                            }
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleUpdateTijarahStatus(item.id, "REJECTED")
                            }
      ...
      {activeTab === "rakanqr" && (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="font-extrabold text-[#0F2028] text-base">
              Permohonan Ahli Duta (Standee Premis)
            </h3>
            <p className="text-xs text-slate-400">
              Pengesahan permohonan duta untuk mengaktifkan kod rujukan,
              standee, dan akaun elaun
            </p>
          </div>
                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            disabled={isApproved}
                            onClick={() =>
                              approveRakanQrAppHandler(item.id, "ACTIVE")
                            }
                          ...
                          >
                            <CheckCircle className="w-3.5 h-3.5" />
                            <span>
                              {isApproved ? "Diluluskan" : "Luluskan"}
                            </span>
                          </button>
      ...
      {activeTab === "admins" && (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="font-extrabold text-[#0F2028] text-base">
              Senarai Staf & Pentadbir Sistem
            </h3>
            <p className="text-xs text-slate-400">
              Pengesahan akaun pentadbir baharu yang mendaftar melalui portal
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-black text-lg text-[#0F2028]">
                  Cipta Projek Kempen Baharu
                </h3>
                <p className="text-xs text-slate-400">
                  Terbitkan projek waqaf untuk sumbangan pewakaf
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={createProjectHandler} className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-[#1A8C4E]"
                  />
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700">
                  Gambar Kempen (Seret & Lepas / Pilih Fail)
                </label>
