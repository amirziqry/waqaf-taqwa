...
export const AdminLoginPage: React.FC<AdminLoginPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRegisterMode) {
      ...
      try {
        await adminRegister(username, fullName, email, phone, password);
        // await api
        ...
        setMessage({
          type: "error",
          text:
            err.response?.data?.message || "Gagal menghantar permohonan admin.",
        });
      } finally {
        setLoading(false);
      }
      ...
      // Admin Login
      try {
        // await api.post('/admin/auth/login', { username, password }).catch(() => null);
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-xs space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-black text-[#0F2028]">
            {isRegisterMode
              ? "Daftar Akses Staf Pentadbir"
              : "Portal Pengurusan Admin"}
          </h2>
        ...
        <div className="text-center pt-2 border-t border-slate-100">
          ...
          >
            {isRegisterMode
              ? "Kembali ke Log Masuk Pentadbir"
              : "Mohon Akses Sebagai Pegawai / Staf Baharu"}
          </button>
        </div>
      </div>
    </div>
