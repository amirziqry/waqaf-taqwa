...
export const TransactionHistoryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadTransactions = async () => {
      try {
        const response = await getAllDonations(0, 100);

        setTransactions(response.donationsList);
      } catch (error) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-slate-100 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Senarai rekod sumbangan dan resit transaksi anda
          </p>
        </div>
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="py-4 px-4 text-slate-500 font-medium">
                    ...
                    {item.paidAt
                      ? (() => {
                          const [date, time] = item.paidAt.split(" ");
