...
export const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const activeRole = localStorage.getItem("wt_user_role") || "user";
    ...
    // Check if user submitted an Ahli Duta application
    const savedRakan = localStorage.getItem("wt_rakan_qr_application");
    if (savedRakan) {
      try {
        const parsed: DutaApplication = JSON.parse(savedRakan);
        if (parsed.membershipTier === "DUTA") {
          setDutaData(parsed);
        }
      } catch {
        // Fallback for invalid local storage
      }
    }
    ...
    //   })
    //   .catch(() => {
    //     // Stored local session remains default
    //   });
    ...
    accountMe()
      .then((res) => {
        if (res?.username) {
          setProfile(res);
          localStorage.setItem("wt_user_name", res.username);
        }
      })
      .catch(() => {
        // Stored local session remains default
    ...
    // 2. Fetch live real-time donation total
    ...
    // api
    //   .get(donationEndpoint)
    //   .then((res) => {
    //     const sum =
    //       typeof res.data === "number" ? res.data : res.data?.total || 0.0;
    //     setTotalDonated(Number(sum));
    //   })
    //   .catch(() => {
    //     try {
    //       const localTransactions = JSON.parse(
    //         localStorage.getItem("wt_transactions") || "[]",
    //       );
    //       const sum = localTransactions.reduce(
  ...
  const getRoleLabel = () => {
    ...
    if (role === "merchant") return "Peniaga";
    return "Pewakaf";
  };

  // Generate public donation URL with the Duta's agent referral parameter
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {profile?.email || "Akaun Terpelihara"}
            </p>
          </div>
        </div>
        ...
        (dutaData.status === "APPROVED" ? (
          ...
          <div className="bg-linear-to-b from-amber-50/70 via-white to-amber-50/30 rounded-3xl p-5 border-2 border-amber-300 shadow-xs space-y-4">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-amber-700/80 font-semibold">
                    Lokasi Premis:{" "}
                    {dutaData.placementLocation || "Kaunter Rasmi"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-white p-3.5 rounded-2xl border border-amber-200/70 flex flex-col justify-between shadow-2xs">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  Pautan Bayaran Terus
                </span>
                <div className="flex items-center justify-between mt-1">
                  <span
                    className="text-[11px] font-semibold text-slate-600 truncate max-w-32.5"
                    title={paymentLink}
                  >
                    {paymentLink}
                  </span>
            ...
            {/* Payout Bank Info */}
            {dutaData.bankName && (
              <div className="flex items-center gap-2 px-3.5 py-2.5 bg-white/80 rounded-xl border border-amber-200/50 text-[11px] text-slate-600">
                <Building className="w-4 h-4 text-amber-700 shrink-0" />
                <span>
                  Akaun Penyaluran Elaun:{" "}
                  <strong className="text-slate-800">
                    {dutaData.bankName}
                  </strong>{" "}
                  (•••• {dutaData.bankAccountNumber?.slice(-4)})
                </span>
              </div>
            )}
          </div>
          ...
          <div className="bg-linear-to-r from-amber-50 to-orange-50 rounded-3xl p-5 border border-amber-200/80 shadow-xs flex items-start gap-3.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-amber-900/80 leading-relaxed">
                Permohonan standee fizikal anda ({dutaData.agentCode}) sedang
                dinilai oleh pentadbir. Pautan bayaran terus dan kod rujukan
                aktif akan dipaparkan di sini selepas kelulusan diberikan.
              </p>
            </div>
          </div>
        ))}
