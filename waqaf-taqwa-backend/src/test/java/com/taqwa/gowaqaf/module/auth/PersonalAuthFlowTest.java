...
public class PersonalAuthFlowTest {
	...
	@BeforeEach
	void setup() {
		CommonClass.createMockPersonal(accountRepository, passwordEncoder, "donator_test", "personal1@gmail.com");
	}
	...
	void donatorCanAccessEndpointAfterLogin() throws Exception {
		...
		mockMvc.perform(get(CommonEndpoints.accountMe).cookie(tokenCookie)).andExpect(status().isOk());
	}

	@Test
	@WithMockPersonal(username = "donator_mock")
	void meEndpointTest() throws Exception {
		Account account = accountRepository.findByUsername("donator_mock").get();
